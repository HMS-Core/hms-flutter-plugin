...
import 'package:huawei_ml/document/ml_document_client.dart';
...
class _DocumentRecognitionPageState extends State<DocumentRecognitionPage> {
  ...
  MlDocumentSettings settings;

  String _recognitionResult = "Result will be shown here.";

  @override
  void initState() {
    settings = new MlDocumentSettings();
    _checkPermissions();
    super.initState();
  }
  ...
  _startRecognition() async {
    try {
      final MlDocument document =
          await MlDocumentClient.getDocumentAnalyzeInformation(settings);
      setState(() {
        _recognitionResult = document.stringValue;
      });
