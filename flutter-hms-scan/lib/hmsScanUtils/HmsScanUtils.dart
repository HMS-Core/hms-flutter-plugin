...
class HmsScanUtils {
  static Future<ScanResponse> startDefaultView(
      DefaultViewRequest request) async {
    final ScanResponse result = ScanResponse.fromJson(await HmsScan
        .instance.scanUtilsChannel
        .invokeMethod("defaultView", request.toMap()));
    return result;
  }

  static Future<Image> buildBitmap(BuildBitmapRequest request) async {
    final Uint8List uInt8list = await HmsScan.instance.scanUtilsChannel
        .invokeMethod("buildBitmap", request.toMap());
...
// Returns Image from Uint8List
  ...
  static Future<ScanResponse> decodeWithBitmap(DecodeRequest request) async {
    final ScanResponse result = ScanResponse.fromJson(await HmsScan
        .instance.scanUtilsChannel
        .invokeMethod("decodeWithBitmap", request.toMap()));
    return result;
  }
