...
import 'package:huawei_drive/src/model/drive_channel.dart';
import 'package:huawei_drive/src/model/drive_file_list.dart';
...
class Files {
  ...
  Future<DriveFileList> list(FilesRequest request) async {
    return DriveFileList.fromJson(
        await _channel.invokeMethod("Files#List", request.toJson()));
  }
  ...
  Future<ExecuteResponse> getContent(FilesRequest request) async {
    return ExecuteResponse.fromJson(
        await _channel.invokeMethod("Files#ExecuteContent", request.toJson()));
  }
  ...
  Future<Int8List> getContentAsInputStream(FilesRequest request) async {
    return Int8List.fromList(await _channel.invokeMethod(
        "Files#ExecuteContentAsInputStream", request.toJson()));
  }
  ...
  Future<bool> getContentAndDownloadTo(
      FilesRequest request, String path) async {
    return await _channel.invokeMethod("Files#ExecuteContentAndDownloadTo", {
      "request": request.toJson(),
      "path": path,
    });
  }
  ...
  Stream<DriveProgress> get onProgressChanged => _progressChannel
      .receiveBroadcastStream()
      .map((event) => DriveProgress.fromMap(Map<String, dynamic>.from(event)));
}
