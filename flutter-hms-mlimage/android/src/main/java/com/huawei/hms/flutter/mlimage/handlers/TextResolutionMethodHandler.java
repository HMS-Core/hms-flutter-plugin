...
public class TextResolutionMethodHandler implements MethodChannel.MethodCallHandler {
    private static final String TAG = TextResolutionMethodHandler.class.getSimpleName();

    private final MLResponseHandler responseHandler;

    private MLTextImageSuperResolutionAnalyzer resolutionAnalyzer;
    ...
    private void getSparseTextResolutionResult(@NonNull SparseArray<MLTextImageSuperResolution> array) {
        List<MLTextImageSuperResolution> list = new ArrayList<>(array.size());
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            MLTextImageSuperResolution textImageSuperResolution = list.get(i);
            map.put(Param.BYTES, Commons.bitmapToByteArray(textImageSuperResolution.getBitmap()));
            mapList.add(map);
        }
        responseHandler.success(mapList);
    }

    private void sendSuccessResult(@NonNull MLTextImageSuperResolution resolution) {
        HashMap<String, Object> resultMap = new HashMap<>();
        resultMap.put(Param.BYTES, Commons.bitmapToByteArray(resolution.getBitmap()));
        responseHandler.success(resultMap);
    }
    
    private void analyzeFrame(MethodCall call) {
        String textResolutionImagePath = FromMap.toString(Param.PATH, call.argument(Param.PATH), false);

        resolutionAnalyzer = MLTextImageSuperResolutionAnalyzerFactory.getInstance().getTextImageSuperResolutionAnalyzer();
        MLFrame frame = SettingCreator.frameFromBitmap(textResolutionImagePath);

        SparseArray<MLTextImageSuperResolution> mlTextImageSuperResolutionSparseArray = resolutionAnalyzer.analyseFrame(frame);
        getSparseTextResolutionResult(mlTextImageSuperResolutionSparseArray);
    }

    private void asyncAnalyzeFrame(MethodCall call) {
        String textResolutionImagePath = FromMap.toString(Param.PATH, call.argument(Param.PATH), false);

        resolutionAnalyzer = MLTextImageSuperResolutionAnalyzerFactory.getInstance()
                .getTextImageSuperResolutionAnalyzer();

        MLFrame frame = SettingCreator.frameFromBitmap(textResolutionImagePath);
        resolutionAnalyzer.asyncAnalyseFrame(frame)
                .addOnSuccessListener(this::sendSuccessResult)
                .addOnFailureListener(responseHandler::exception);
    }

    private void stop() {
        if (resolutionAnalyzer == null) {
            responseHandler.noService();
            return;
        }
        resolutionAnalyzer.stop();
