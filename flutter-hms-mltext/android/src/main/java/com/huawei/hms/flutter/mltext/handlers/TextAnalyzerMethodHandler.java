...
public class TextAnalyzerMethodHandler implements MethodChannel.MethodCallHandler, IAnalyzer {
    ...
    private void asyncAnalyseFrame(@NonNull MethodCall call) {
        ...
        if (isRemote) {
            txtAnalyzer = MLAnalyzerFactory.getInstance().getRemoteTextAnalyzer(SettingCreator.createRemoteTextSetting(call));
        } else {
            txtAnalyzer = MLAnalyzerFactory.getInstance().getLocalTextAnalyzer(SettingCreator.createLocalTextSetting(call));
        }
